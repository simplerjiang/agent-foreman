...
- [x] **T0.1** 增量 JSON **早切器**（新 shared util）：括号深度 + **字符串转义/代码围栏**处理，"第一个完整顶层对象 **且** 通过字段校验即收口并 abort 流"。｜依赖：无｜建议认领：claude-code｜**验收**：单测覆盖多对象串联 / 转义内 `{}` / ```json 围栏 / 前后噪声，均返回第一个合法对象、不误切。
- [x] **T0.2** 修脆弱解析：`_extract_json_object`（`pm_agent.py:159`）改"取**第一个**平衡对象"，并复用到 `tools/loop.py:267-288`。｜依赖：T0.1｜建议认领：claude-code｜**验收**：47 份串联 JSON 输入 → 取到第 1 份合法对象、不再降级 fallback。
- [x] **T0.3** PM **LLM 调用级看门狗**（`client.py` ws/HTTP 外层）：(a) 整轮墙钟 `T_wall`(60–90s)；(b) 结构化复读检测（完整对象后再现新对象/指纹重复）；(c) 无进展 `T_stall`(15–30s)。命中 → abort + **关 ws** + 抛可识别异常。｜依赖：无（与 T0.1 并行）｜建议认领：codex｜**验收**：mock"持续吐 delta、永不 completed"的流 → `T_wall` 内 abort。
- [x] **T0.4** **会话状态机**：新增/明确 `stalled` 终态（`dispatch_service.py:48-49`），把看门狗 abort 映射到 `failed`/`stalled` 并落 reason 事件。｜依赖：T0.3｜建议认领：codex｜**验收**：触发看门狗 → 会话不再 `running`，DB 有终态 + reason。
- [x] **T0.5** **UI 可见**（`app.js`）：渲染 `stalled`/`failed` + reason，running 不再永久。｜依赖：T0.4｜建议认领：claude-code｜**验收**：E2E 真机看到失败原因，非空转。
- [x] **T0.6** **覆盖 review/compact**：T0.1–T0.3 的早切+墙钟+容错应用到 `REVIEW_SYSTEM`/`COMPACT_SYSTEM` 路径（`pm_agent.py:657-680`）。｜依赖：T0.1–T0.3｜建议认领：claude-code｜**验收**：review/compact 同样能在第一个对象收口、能墙钟失败。
- [ ] **P0 验收（合）**：① 复读流在第一个合法对象处收口并继续 dispatch；② 无 completed 流在 `T_wall` 内失败 + UI 可见 reason；③ 四路（plan/loop/review/compact）均覆盖。

### P1 — 根治：A1 强制工具（ws）

- [x] **T1.1** `client.py` ws `response.create` 帧支持透传 `tools` + `tool_choice`（`_responses_ws_once`，`client.py:427`）。｜依赖：无｜建议认领：codex｜**验收**：抓帧确认 `tools/tool_choice` 已发；对照附录 B 探针格式。
- [x] **T1.2** `client.py` ws **解析 function-call 事件**：`output_item.added(function_call)` + `function_call_arguments.delta` 累加 + `...done` → 装配 `LLMToolCall`；以 function_call done/`completed` 为终结。｜依赖：T1.1｜建议认领：codex｜**验收**：对真实 ws（或回放附录 B 事件流）能还原出完整 `{name, arguments}`。
- [x] **T1.3** 去掉 `tool_complete()` 的 ws 纯文本短路（`client.py:207-211`）→ ws 返回真实 `tool_calls`。｜依赖：T1.2｜建议认领：codex｜**验收**：ws 路径 `tool_complete` 返回非空 `tool_calls`。
- [x] **T1.4** 定义 `submit_plan` 工具 schema（§5）接入 `PMToolLoop`：取证轮 `tool_choice:auto`，**提交轮强制 `submit_plan`**。｜依赖：T1.3｜建议认领：claude-code｜**验收**：取证轮仍能调 `read_file/search_repo`；提交轮只产一次 `submit_plan`。
- [x] **T1.5** `parse_plan` 改吃 `tool_call.arguments`（dict 直入），**删事后正则路径**；`loop.py` 同步。｜依赖：T1.4｜建议认领：claude-code｜**验收**：`PMPlan` 字段语义不变；正则路径无残留。
- [x] **T1.6** **回归** plan→dispatch→review 全链路 + tool_pre/tool_post 顺序（`tests/test_dispatch_service.py:639-647`）。｜依赖：T1.5｜建议认领：claude-code｜**验收**：既有用例全绿。
- [ ] **P1 验收（合）**：诱发复读的输入 → **只产生一次 `submit_plan` 调用、秒级终结、正常 dispatch**；`foreman-e2e` 真机跑一次"审阅代码"任务确认（基调 §8）。

### P2 — UX

- [x] **T2.1** 计划**增量可见**：todo 随提交渲染（`app.js` `TodoPanel`）。｜依赖：P1｜认领：claude-code｜**落地**：`TodoPanel` 本地 `revealed` 计数 + `fdup` 入场动画，todos 从 0→N 增长时逐条渐显；满载重开的会话直接全显、不重放动画。｜**验收**：E2E 看到 todo 渐显。
- [x] **T2.2** plan 阶段**实时状态**：reasoning 流 + "已 N 秒"。｜依赖：P1｜认领：claude-code｜**落地**：`pm-status` 节点挂自走秒表组件 `PmElapsed`（`start=节点首现 ts`，每秒自渲染），即使无新 delta 也显示"已 N 秒"，规划期不再像冻屏。Stop 按钮见 T2.3（live 时 workspace 头部「取消会话」）。｜**验收**：plan 中界面有进度、非白屏。
- [x] **T2.3** **Stop 真取消**正在跑的 ws 调用，不只标 `cancelled`（`dispatch_service.py` `cancel`/`_cancel_session_tasks`）。｜依赖：T0.3、T2.2｜认领：claude-code｜**落地**：`cancel()` 标 `cancelled` 后调 `_cancel_session_tasks` 取消该会话已追踪的 launch task；CancelledError 经 `ws.recv()` 抛入 `_responses_ws_once`，`async with ws` 退出即关闭 socket。CancelledError 为 BaseException，`_safe_*` 的 `except Exception` 不吞，task 落 cancelled。单测 `test_cancel_aborts_in_flight_pm_call`。｜**验收**：点 Stop → ws 实际断、会话即停（真机 E2E 待复验）。

### P3 — 收尾

- [x] **T3.1** 删死路径（旧正则解析残留）。｜依赖：P1｜**落地**：随 T0.2/T1.5 已根除"首 `{`→末 `}`"正则截取——全仓 PM 解析器（plan/review/compact/auditor/briefing/operator/reviewer/`tools.loop`）统一委托 `shared.jsonscan.first_json_object`；`client` 下再无 JSON 抽取相关 `re`/`rfind("}")`。`loop.py` 残留的结构化 `final_plan` 文本协议**非死路径**，是 `complete()` 非工具传输的 back-compat（既有单测仍覆盖、纵深防御 §0.5-3），保留。｜**验收**：无 dead code（旧正则路径已无残留）。
- [x] **T3.2** 补单测 / 文档；§7 验收逐项核对。｜依赖：P1/P2｜**落地**：新增 `test_cancel_aborts_in_flight_pm_call`；本设计书 §6.5 勾选与 §11.1 验收对照更新。代码可证项 A/B/D/F 见 §11.1；E/G 需真机 E2E（T3.3）。｜**验收**：见 §11.1 对照表。
- [ ] **T3.3** 二次修复纪律：本设计涉及曾被 E2E 打回的项（#39），按 `AGENTS.md` §二**当场 E2E 复验再关**。｜依赖：相关项落地后｜**验收**：附 🖱️实测证据 + commit SHA。**当前状态：#39 保持 open + needs-e2e，未真机 E2E 通过前不关。**
...
## 11.1 验收对照（§7 A–G，2026-06-25 P2/P3 落地后）

> 供 codex code-review 对照真实代码核验。代码可证项已逐条标 ✅；E/G 涉及真机点击，须 `foreman-e2e` 复验（T3.3）。

| 项 | 状态 | 代码证据 |
|---|---|---|
| **A** 复读不可发生 | ✅ 代码可证 | 提交轮 `tool_choice=submit_plan` 强制单工具（`tools/loop.py` `force_submit`/`_SUBMIT_PLAN_CHOICE`），turn 收一个 tool_use 即终结；ws 文本路另有 `_StructuredRepeatWatch`（`client.py`）。单测覆盖强制轮、复读早切。 |
| **B** 终结不靠文本猜测 | ✅ 代码可证 | 原生路：`_submit_plan_args(calls)` 直取工具参数为计划，无正则；文本路早切取**第一个**合法对象（`first_json_object`）。无"首{→末}"正常路径。 |
| **C** 挂死可自愈 | ✅ 代码可证（真机待复验） | `_responses_ws_once` 墙钟+无进展+复读看门狗 → `LLMStalledError`；`_safe_pm_launch` 映射 `stalled`+reason；前端 `app.js` 红标 + `friendlyReason`。 |
| **D** 解析器健壮 | ✅ 代码可证 | `shared/jsonscan.py` `_iter_objects` 处理串联/围栏/转义/噪声；`test_jsonscan`/`test_context_compression` 覆盖。 |
| **E** UX 可见 | ⏳ 半证（需 E2E） | T2.1 `TodoPanel` 渐显、T2.2 `PmElapsed` 计时、T2.3 真 Stop、tool_pre/post 实时渲染——逻辑已在；"真窗口看到"须 E2E。 |
| **F** 无回归 | ✅ 测试可证 | 既有 `test_dispatch_service`/`test_pm_tools` 全绿（含 tool_pre/tool_post 先于 pm_plan）；`PMPlan` 字段语义不变。 |
| **G** 真机证据 | ⏳ 待 E2E | 按 `foreman-e2e` 在 main/exe 真机复验 plan 路径（#39），附实测/截图/SHA。 |

---

## 11.2 codex 评审第二轮处置（2026-06-25，P2/P3 落地后）

**评审范围**：`git diff 0dac0ea..HEAD`（claude-code 在 codex 底座之上的完整 lane：P0/P1 + P2/P3）。**结论：有条件通过**——任务一 4 条旧结论经现树复核**全部 ✅ 已接入**（确认上一轮命中过期 checkout）；P2/P3 核心（cancel/elapsed/reveal）**未发现缺陷**（T2.3 cancel 顺序/`CancelledError` 不被吞/`_session_tasks` 清理均已核实）。3 条 Medium/Low 已逐条处置：

| 评审项 | 级别 | 处置 |
|---|---|---|
| 文本 `final_plan` 仍可终结，非纯 A1（§0.5-1/§11.1-B） | Medium | ✅ 已收紧：`_complete` 回传 `native` 标志；`loop.py` **仅在非原生传输**（无 native tool call）接受文本 `final_plan` 终结，生产 ws/A1 路径必须走 `submit_plan` tool call。新测 `test_pm_loop_native_path_ignores_text_final_plan`。 |
| `validate_final_plan` 未本地 enforce §5 schema 边界 | Medium | ✅ 已加：summary≤600 / model≤80 / instruction≤6000 / todo≤12×200 / deliberation≤8×300 本地夹断。新测 `test_validate_final_plan_clamps_schema_bounds`。 |
| `TodoPanel` `revealed` 未按 session key，跨会话可重放历史 todo | Low | ✅ 已修：`TodoPanel` 按 `sessionRow.id` 加 `key`，切会话即重挂载、`revealed` 重置。 |

**E2E 就绪**：codex 判定**当前具备进入真机 E2E 验收的条件**；E2E 重点验 ws 强制 `submit_plan`、Stop 真取消、stalled reason 可见（§7-E/G、T3.3）。`#39` 保持 open + `needs-e2e`，真机 E2E 通过前不关。
