...
> 💬 **人话：上面几个安全词**
> - **Bearer Token**：一串只有你知道的"通行密码"，手机每次连服务器都要带上它，别人没有就进不来。
> - **nonce / 防重放**：一次性口令，用过就作废。防止别人录下你"批准"那条请求，回头重发一遍冒充你。
> - **纵深防御**：多设几道关卡。就算第一道（密码）被攻破，危险动作还得过"闸门"这道关，光偷到密码也删不了你的库。
> - **Secrets（机密）**：API key、私钥这类绝不能外泄的东西，只放在本地 `.env` 文件里，绝不上传 GitHub。
