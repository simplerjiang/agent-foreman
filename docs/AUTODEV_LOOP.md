...
**全自主模式**（用户 2026-06-20 授权）：`PAUSE_AT_PHASE=false` + `TARGET=到 P7 结束` = 一路做完整个 roadmap、不在阶段边界停。中途自己解决问题、做合理决策并在 commit/TASKS 记录；缺凭据的（用户 LLM key、live 多租户上线等）用 mock/本地测、**延后真接入并标注**，不阻塞。**只在「极度危害/不可逆」或「真卡死」时才停下问**（见铁律「升级红线」）。全部做完才汇总 + 列延后项。
...
- **升级红线（全自主模式下，何时才停下问你）**：仅当遇到**极度危害 / 不可逆**操作——动服务器 :80 网关、删数据 / 删服务器、开 usage-credits / 花钱、`git push --force`、`rm -rf` 真实目录、改用户的别的项目、泄露 / 提交机密——或**真正不可恢复的卡死**。其余一律自己决断、记录、继续，不打断。
