...
3. `policy_context`: 哪些工具已开启、哪些动作需要审批、哪些命令/域名/路径在 allowlist 内。
...
```json
{
  ...
  "policy_context": {
    ...
    "allowed_commands": [
      {"command": "python --version", "purpose": "check Python runtime"},
      {"command": "git status --short", "purpose": "inspect worktree"}
    ],
    "requires_approval": [
      "write_file",
      "replace_in_file",
      "run_command outside allowlist",
      "browser form submit",
...
- 如果 PM loop 已经引入 `external_web_content`, 后续 shell 默认 `requires-approval`, 除非命令在明确 allowlist 内, 例如只读版本查询或项目内测试命令。
...
- `web_search` 产生 `external_web_content` taint 后, 非 allowlist shell 被降级为 `requires-approval`。
...
| `run_command` | 运行 allowlist 命令 | yes | yes | yes | yes | exit 0 stdout='Python 3.12.10' | `returncode=0; stdout=Python 3.12.10\n; stderr=` |
