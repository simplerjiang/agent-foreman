...
async def test_create_persists_and_activates(tmp_path):
    ...
    res = await svc.create_definition(kind="workflow", name="add-feature", body="steps: []")
...
async def test_create_emits_definition_event(tmp_path):
    ...
    res = await svc.create_definition(kind="skill", name="how-to-test", body="x")
    seen = _drain(q)
...
async def test_create_autobumps_version(tmp_path):
    svc, _ = _svc(tmp_path)
    a = await svc.create_definition(kind="skill", name="s", body="v1")
    b = await svc.create_definition(kind="skill", name="s", body="v2")
...
async def test_create_explicit_duplicate_version_conflicts(tmp_path):
    svc, _ = _svc(tmp_path)
    await svc.create_definition(kind="skill", name="s", version=1, body="a")
    res = await svc.create_definition(kind="skill", name="s", version=1, body="b")
    assert res == {"ok": False, "error": "version_exists"}


async def test_create_no_activate_leaves_previous_live(tmp_path):
    svc, _ = _svc(tmp_path)
    v1 = await svc.create_definition(kind="qa_rubric", name="r", body="v1")
    v2 = await svc.create_definition(kind="qa_rubric", name="r", body="v2", activate=False)
...
async def test_create_rejects_unknown_kind(tmp_path):
    svc, _ = _svc(tmp_path)
    res = await svc.create_definition(kind="prompt-template", name="x", body="y")
    assert res == {"ok": False, "error": "bad_kind"}


async def test_create_rejects_blank_name(tmp_path):
    svc, _ = _svc(tmp_path)
    assert (await svc.create_definition(kind="skill", name="   ", body="y"))["error"] == "bad_name"


async def test_create_rejects_oversize_body(tmp_path):
    svc, _ = _svc(tmp_path)
    res = await svc.create_definition(kind="skill", name="x", body="a" * 200_001)
    assert res == {"ok": False, "error": "body_too_large"}


async def test_create_rejects_non_object_scope_json(tmp_path):
    svc, _ = _svc(tmp_path)
    res = await svc.create_definition(kind="skill", name="x", body="y", scope_json="[1,2]")
    assert res == {"ok": False, "error": "bad_scope_json"}


async def test_create_rejects_invalid_metadata_json(tmp_path):
    svc, _ = _svc(tmp_path)
    res = await svc.create_definition(kind="skill", name="x", body="y", metadata_json="{bad")
    assert res == {"ok": False, "error": "bad_metadata_json"}


async def test_create_no_store_errors():
    svc = DefinitionService(None)
    assert (await svc.create_definition(kind="skill", name="x", body="y"))["error"] == "no_store"


# ── read / list ──────────────────────────────────────────────────────────────────────────────────
async def test_list_filters_by_kind(tmp_path):
    svc, _ = _svc(tmp_path)
    await svc.create_definition(kind="workflow", name="w", body="a")
    await svc.create_definition(kind="skill", name="s", body="b")
...
async def test_update_edits_body_in_place(tmp_path):
    svc, store = _svc(tmp_path)
    d = (await svc.create_definition(kind="skill", name="s", body="old"))["definition"]
...
async def test_update_rejects_bad_status(tmp_path):
    svc, _ = _svc(tmp_path)
    d = (await svc.create_definition(kind="skill", name="s", body="x"))["definition"]
...
async def test_activate_makes_exactly_one_live(tmp_path):
    svc, _ = _svc(tmp_path)
    v1 = (await svc.create_definition(kind="workflow", name="w", body="v1"))["definition"]
    (await svc.create_definition(kind="workflow", name="w", body="v2"))  # v2 now live
    # roll back to v1
...
async def test_delete_emits_event_with_identity(tmp_path):
    ...
    d = (await svc.create_definition(kind="code_standard", name="cs", body="x"))["definition"]
