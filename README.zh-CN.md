...
[路线图](docs/ROADMAP.md) 推进：P1 单机驱动 → P2 审阅+检查点 → P3 手机+审批 →
P4 决策回路（操作员+审核员+决策卡）→ P5 定义引擎（开放内核「秘方」）→ P6 UI 编辑器 → P7 团队/中继模式。

支持**个人**（本地进程自开 PWA）或**团队**（一台共用服务器当总机，多人各自本地进程用 access key 接入）两种模式，
同一套代码。详见 [DESIGN.zh-CN.md §8](docs/DESIGN.zh-CN.md)。
